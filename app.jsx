// React globals injected from CDN
...
// ─────────────────────────────────────────
function LoginScreen({ role, onSuccess, onBack, accounts: accountsProp }) {
  ...
  const doLogin = () => {
    ...
    setTimeout(() => {
      if (isAdmin) {
        ...
        // Use accounts passed as prop (from React state) OR read fresh from localStorage
        const accounts = (accountsProp && accountsProp.length > 0)
          ? accountsProp
          : loadRestaurantAccounts();

        const userInput = user.trim().toLowerCase();
        const passInput = pass;
        const byUser = accounts.find(a => (a.user||"").toLowerCase().trim() === userInput);

        if(accounts.length === 0) {
          setError("No hay cuentas registradas. Crea una desde Admin → Restaurantes → 🔑 Cuentas.");
          setShake(true); setTimeout(()=>setShake(false),600);
        } else if(!byUser) {
          const available = accounts.map(a=>a.user).join(", ");
          setError(`Usuario no encontrado. Disponibles: ${available}`);
          setShake(true); setTimeout(()=>setShake(false),600);
        } else if(byUser.pass !== passInput) {
          setError("Contraseña incorrecta. Verifica mayúsculas y espacios.");
          setShake(true); setTimeout(()=>setShake(false),600);
        } else if(byUser.active === false || byUser.active === "false" || byUser.active === 0) {
          setError("Cuenta desactivada. Actívala desde el Panel de Admin.");
          setShake(true); setTimeout(()=>setShake(false),600);
        } else {
          const s = {
            role: "restaurant",
            user: byUser.user,
            name: byUser.restaurantName,
            restaurantName: byUser.restaurantName,
            restaurantEmoji: byUser.emoji || "🍽️",
            restaurantId: byUser.restaurantId || String(byUser.id),
          };
          setSession(s);
          onSuccess(s);
        }
    ...
    <div style={{ background:"#F9FAFB", minHeight:"100vh", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px" }}>
        ...
        <div style={{ marginTop:32,background:"#fff",borderRadius:16,padding:"14px 18px",width:"100%",maxWidth:360,border:"1.5px solid #F3F4F6" }}>
          ...
          {isAdmin
            ? "Solo el dueño de la plataforma tiene acceso a este panel."
            : isDriver
            ? "Tu usuario y contraseña fueron asignados por el administrador."
            : (()=>{
                const accounts = (accountsProp && accountsProp.length > 0) ? accountsProp : loadRestaurantAccounts();
                return accounts.length > 0
                  ? `Cuentas: ${accounts.map(a=>`${a.restaurantName} → ${a.user}`).join(" · ")}`
                  : "⚠️ Sin cuentas registradas. Ve a Admin → Restaurantes → 🔑 Cuentas → ＋ Cuenta.";
              })()
          }
          </div>
          {/* Quick fix button for restaurant - migrate any existing account */}
          {isRestaurant&&(()=>{
            const accounts = loadRestaurantAccounts();
            if(accounts.length > 0) return null;
            // Check if there's data in old format
            const oldData = (() => { try { return JSON.parse(localStorage.getItem("yg_rest_accounts")||"null"); } catch { return null; } })();
            return (
              <div style={{ marginTop:10,padding:"10px 12px",background:"#FFF7ED",borderRadius:10,fontSize:12,color:"#F97316",fontWeight:600 }}>
                Si ya creaste una cuenta en el admin, intenta volver al panel de admin, ir a Restaurantes → 🔑 Cuentas y crear la cuenta nuevamente.
              </div>
            );
          })()}
        </div>
...
// ─────────────────────────────────────────
// LOCAL ORDERS STORAGE (works without DB)
// ─────────────────────────────────────────
const LOCAL_ORDERS_KEY = "yg_local_orders";
const loadLocalOrders  = () => { try { return JSON.parse(localStorage.getItem(LOCAL_ORDERS_KEY)||"[]"); } catch { return []; } };
const saveLocalOrders  = (list) => { localStorage.setItem(LOCAL_ORDERS_KEY, JSON.stringify(list)); bumpSync(); };
...
// Same-browser tabs: StorageEvent (instant)
// Cross-device: Supabase polling every 3s
// ─────────────────────────────────────────
const SYNC_KEY      = "yg_sync_ts";
const REMOTE_ORDERS_TS_KEY = "yg_remote_ts"; // last seen remote timestamp

const bumpSync = () => {
  const ts = String(Date.now());
  try { localStorage.setItem(SYNC_KEY, ts); } catch(e) {}
  try { const bc = new BroadcastChannel("yg"); bc.postMessage({type:"sync",ts}); bc.close(); } catch(e) {}
};

// Fetch latest orders from Supabase and merge with local
const syncOrdersFromRemote = async () => {
  try {
    const remote = await sbGet("orders","order=created_at.desc&limit=200");
    ...
    let changed = false;
    remote.forEach(ro => {
      const key = String(ro.id);
      const existing = localMap.get(key);
      // Add new or update status of existing
      if(!existing || existing.status !== ro.status) {
        localMap.set(key, {...(existing||{}), ...ro});
        changed = true;
      }
    });
    if(changed) {
...
// Also push local-only orders up to Supabase
const pushLocalOrdersToRemote = async () => {
  const local = loadLocalOrders();
  for(const order of local) {
    if(!order.db_synced) {
      try {
        const res = await sbPost("orders", order);
        if(res?.[0]?.id) {
          updateLocalOrder(order.id, {db_synced:true, db_id:res[0].id});
        }
      } catch {}
    }
  }
...
// Hook: fires callback whenever any data changes
function useLiveSync(callback, intervalMs=3000) {
  useEffect(() => {
    // 1. BroadcastChannel — instant, same browser profile
    let bc;
    try {
      bc = new BroadcastChannel("yg");
      bc.onmessage = (ev) => { if(ev.data?.type==="sync") callback(); };
    } catch(e) {}

    // 2. Storage event — other tabs same origin
    const onStorage = (e) => {
      if([SYNC_KEY, LOCAL_ORDERS_KEY, "yg_local_restaurants","yg_local_menu_items"].includes(e.key))
        callback();
    };
    window.addEventListener("storage", onStorage);

    // 3. Supabase polling — cross-device (different phones/computers)
    const poll = setInterval(async () => {
      const changed = await syncOrdersFromRemote();
      if(changed) callback();
      else callback(); // always refresh from localStorage
    }, intervalMs);
...
// Lightweight hook for menu/restaurant sync only
function useMenuSync(callback, intervalMs=8000) {
  useEffect(() => {
    let bc;
    try {
      bc = new BroadcastChannel("yg_menu");
      bc.onmessage = () => callback();
    } catch(e) {}
    const onStorage = (e) => {
      if(["yg_local_restaurants","yg_local_menu_items"].includes(e.key)) callback();
    };
    window.addEventListener("storage", onStorage);
    const poll = setInterval(callback, intervalMs);
    return () => { bc?.close(); window.removeEventListener("storage",onStorage); clearInterval(poll); };
  }, []);
}

const bumpMenuSync = () => {
  try { const bc = new BroadcastChannel("yg_menu"); bc.postMessage("sync"); bc.close(); } catch(e) {}
};
...
function CheckoutScreen({ restaurant, onBack, onOrderPlaced, initialPromo=null, allMenuItems=[] }) {
  ...
  useEffect(()=>{
    // 1. Use items passed from App state (already includes admin additions)
    const fromProps = allMenuItems.filter(i=>i.restaurant_id===restaurant.id && i.available!==false);
    if(fromProps.length > 0) {
      setMenuItems(fromProps);
      setLoadingMenu(false);
      return;
    }
    // 2. Try local storage
    const localItems = (() => { try { return JSON.parse(localStorage.getItem("yg_local_menu_items")||"null"); } catch { return null; } })();
    if(localItems) {
      const filtered = localItems.filter(i=>i.restaurant_id===restaurant.id && i.available!==false);
      if(filtered.length > 0) { setMenuItems(filtered); setLoadingMenu(false); return; }
    }
    // 3. Try DB
  ...
  const placeOrder = async () => {
    ...
    const deliveryAddress = (()=>{
      ...
      return a ? `${a.street} #${a.number}, ${a.colonia}, CP ${a.cp}` : "—";
    })();
    const paymentMethod = payments.find(p=>p.id===selPay)?.label || "—";

    const orderPayload = {
      restaurant_id: restaurant.id,
      restaurant_name: restaurant.name,
      restaurant_emoji: restaurant.emoji,
      ...
      customer_name: customerSession?.name || "Invitado",
      customer_phone: customerSession?.phone || "",
    };

    // Always save locally first (works without DB)
    const localOrders = loadLocalOrders();
    const localId = Date.now();
    const localOrder = { ...orderPayload, id: localId, db_synced: false };
    saveLocalOrders([localOrder, ...localOrders]);
    bumpSync();

    localStorage.setItem("hasOrdered","true");

    // Push to Supabase immediately so other devices see it
    sbPost("orders", orderPayload).then(res => {
      if(res?.[0]?.id) {
        updateLocalOrder(localId, { db_synced: true, db_id: res[0].id });
        bumpSync(); // notify other devices
      }
    }).catch(()=>{});

    onOrderPlaced(localId);
    setPlacing(false);
  };
            ...
            <div key={cat}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                {items.map(item=>{
                  ...
                  return (
                    <div key={item.id} style={{ background:"#fff",borderRadius:16,padding:"14px 16px",boxShadow:"0 1px 8px rgba(0,0,0,0.05)",display:"flex",alignItems:"flex-start",gap:12 }}>
                      <div style={{ flex:1 }}>
                        {/* Promo/type badges */}
                        <div style={{ display:"flex",gap:5,flexWrap:"wrap",marginBottom:3 }}>
                          {item.itemType==="drink"&&<span style={{ background:"#EEF2FF",color:"#6366F1",fontSize:10,fontWeight:700,padding:"1px 7px",borderRadius:99 }}>🥤 Bebida</span>}
                          {item.itemType==="combo"&&<span style={{ background:"#FFF7ED",color:"#F97316",fontSize:10,fontWeight:700,padding:"1px 7px",borderRadius:99 }}>🎁 Combo</span>}
                          {item.promoType==="2x1"&&<span style={{ background:"#FF4D4D",color:"#fff",fontSize:10,fontWeight:800,padding:"1px 7px",borderRadius:99 }}>2×1</span>}
                          {item.promoType==="descuento"&&item.discountPct&&<span style={{ background:"#F97316",color:"#fff",fontSize:10,fontWeight:800,padding:"1px 7px",borderRadius:99 }}>{item.discountPct}% OFF</span>}
                          {item.isSpicy&&<span style={{ fontSize:11 }}>🌶️</span>}
                          {item.isVegan&&<span style={{ fontSize:11 }}>🌱</span>}
                        </div>
                        <div style={{ fontSize:14,fontWeight:700,color:"#111" }}>{item.name}</div>
                        {item.size&&<div style={{ fontSize:11,color:"#9CA3AF",marginTop:1 }}>{item.size}</div>}
                        {item.description&&<div style={{ fontSize:12,color:"#9CA3AF",marginTop:2,lineHeight:1.4 }}>{item.description}</div>}
                        {item.comboItems&&<div style={{ fontSize:11,color:"#F97316",marginTop:3,lineHeight:1.5,whiteSpace:"pre-line" }}>{item.comboItems}</div>}
                        <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:5 }}>
                          <span style={{ fontSize:15,fontWeight:900,color:"#FF4D4D" }}>${item.price}</span>
                          {item.preparationTime&&<span style={{ fontSize:11,color:"#9CA3AF" }}>🕐 {item.preparationTime}</span>}
                          {item.calories&&<span style={{ fontSize:11,color:"#9CA3AF" }}>🔥 {item.calories} kcal</span>}
                        </div>
                      </div>
...
function OrdersScreen({ onTrack }) {
  ...
  useEffect(()=>{
    ...
    const local = loadLocalOrders();
    if(local.length > 0) { setOrders(local); setLoading(false); return; }
    // Try DB fallback
    sbGet("orders","order=created_at.desc&limit=20")
...
function DriverScreen() {
  ...
  const loadDriverOrders = (showRefresh=false) => {
    if(showRefresh) setRefreshing(true);
    // Load all orders from localStorage
    const allOrders = loadLocalOrders();
    // Active orders (pending/delivering for this driver)
    const active = allOrders.filter(o=>["pending","confirmed","delivering"].includes(o.status));
    setOrders(active);
    // Delivered stats
    const delivered = allOrders.filter(o=>o.status==="delivered");
    setEarnings(delivered.reduce((s,o)=>s+(o.delivery_fee||25),0));
    setDeliveredCount(delivered.length);
    setLoading(false);
    if(showRefresh) setTimeout(()=>setRefreshing(false),600);
    // Also try DB
    sbGet("orders","status=in.(pending,confirmed,delivering)&order=created_at.desc")
  ...
  useEffect(()=>{
    ...
    const poll = setInterval(()=>loadDriverOrders(false), 8000);
  ...
  const updateStatus = async (order, newStatus) => {
    ...
    if(newStatus==="delivered") {
      ...
      setEarnings(e=>e+(order.delivery_fee||25));
    ...
    // Try DB
    sbPatch("orders",order.id,{status:newStatus}).catch(()=>{});
  };
        ...
        {selected?(
          ...
          <>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16 }}>
              <div>
                <div style={{ fontSize:13,color:"#9CA3AF",fontWeight:500 }}>Panel 👋</div>
                <div style={{ fontSize:20,fontWeight:900,color:"#111" }}>Repartidor</div>
              </div>
              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                {/* Live badge */}
                <div style={{ display:"flex",alignItems:"center",gap:5,background:"#F0FDF4",border:"1.5px solid #BBF7D0",borderRadius:99,padding:"5px 12px" }}>
                  <div style={{ width:7,height:7,borderRadius:99,background:"#16A34A",opacity:pulse?1:0.3,transition:"opacity 0.5s" }}/>
                  <span style={{ fontSize:11,fontWeight:700,color:"#16A34A" }}>En vivo</span>
                </div>
                {/* Refresh */}
                <div onClick={()=>loadDriverOrders(true)} style={{ width:36,height:36,borderRadius:11,background:refreshing?"#F0FDF4":"#F9FAFB",border:"1.5px solid #F3F4F6",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:15,transition:"all 0.2s" }}>
                  {refreshing?"✓":"🔄"}
                </div>
                {/* Online toggle */}
                <div onClick={()=>setOnline(o=>!o)} style={{ display:"flex",alignItems:"center",gap:6,background:online?"#F0FDF4":"#F9FAFB",border:`1.5px solid ${online?"#16A34A":"#E5E7EB"}`,borderRadius:99,padding:"5px 12px",cursor:"pointer" }}>
                  <div style={{ width:8,height:8,borderRadius:99,background:online?"#16A34A":"#D1D5DB",boxShadow:online&&pulse?"0 0 0 3px rgba(22,163,74,0.25)":"none" }}/>
                  <span style={{ fontSize:12,fontWeight:700,color:online?"#16A34A":"#9CA3AF" }}>{online?"Activo":"Inactivo"}</span>
                </div>
              </div>
            </div>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8 }}>
              {[
                {icon:"📦",label:"Activos ahora",  value:orders.length},
                {icon:"✅",label:"Entregados",      value:deliveredCount},
                {icon:"💰",label:"Ganancias",       value:`$${earnings}`},
              ].map((s,i)=>(
...
// ─────────────────────────────────────────
// ADMIN CLIENTES TAB — proper React component
// ─────────────────────────────────────────
function AdminClientesTab({ orders }) {
  const [allCustomers, setAllCustomers] = useState(()=>loadCustomers());
  const [custSearch, setCustSearch]     = useState("");
  const [selectedCust, setSelectedCust] = useState(null);

  useEffect(()=>{ setAllCustomers(loadCustomers()); },[]);
  ...
  const custOrders = (c) => (orders||[]).filter(o=>
    (o.delivery_address||"").toLowerCase().includes((c.address?.colonia||"___").toLowerCase())
  );

  const toggleActive = (id) => {
    const updated = allCustomers.map(c=>c.id===id?{...c,active:c.active===false?true:false}:c);
    saveCustomers(updated); setAllCustomers(updated);
    if(selectedCust?.id===id) setSelectedCust(p=>({...p,active:p.active===false?true:false}));
  };

  const deleteCustomer = (id) => {
    const updated = allCustomers.filter(c=>c.id!==id);
    saveCustomers(updated); setAllCustomers(updated); setSelectedCust(null);
  };
  ...
  if(selectedCust) {
    ...
    return (
      <div style={{ padding:"16px 24px",display:"flex",flexDirection:"column",gap:12,paddingBottom:100 }}>
        <button onClick={()=>setSelectedCust(null)} style={{ alignSelf:"flex-start",background:"#F9FAFB",border:"1.5px solid #F3F4F6",borderRadius:10,padding:"7px 16px",fontSize:13,fontWeight:700,cursor:"pointer",color:"#374151",fontFamily:"inherit" }}>← Volver</button>

        <div style={{ background:"linear-gradient(135deg,#FF4D4D,#FF6B6B)",borderRadius:20,padding:"20px 22px",boxShadow:"0 4px 20px rgba(255,77,77,0.25)" }}>
          <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:14 }}>
            <div style={{ width:56,height:56,borderRadius:99,background:"rgba(255,255,255,0.25)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:900,color:"#fff",flexShrink:0 }}>
              {(selectedCust.name?.[0]||"?").toUpperCase()}
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:18,fontWeight:900,color:"#fff" }}>{selectedCust.name}</div>
              <div style={{ fontSize:13,color:"rgba(255,255,255,0.8)" }}>📱 {selectedCust.phone}</div>
              {selectedCust.email&&<div style={{ fontSize:12,color:"rgba(255,255,255,0.65)" }}>✉️ {selectedCust.email}</div>}
            </div>
            <div style={{ background:selectedCust.active!==false?"rgba(255,255,255,0.2)":"rgba(0,0,0,0.2)",borderRadius:99,padding:"4px 12px",fontSize:11,fontWeight:700,color:"#fff" }}>
              {selectedCust.active!==false?"✓ Activo":"✗ Inactivo"}
            </div>
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8 }}>
            {[{l:"Pedidos",v:selectedCust.totalOrders||co.length},{l:"Gastado",v:`$${cRev}`},{l:"Registro",v:new Date(selectedCust.createdAt).toLocaleDateString("es-MX",{day:"numeric",month:"short"})}].map((s,i)=>(
              <div key={i} style={{ background:"rgba(255,255,255,0.2)",borderRadius:10,padding:"10px",textAlign:"center" }}>
        ...
        {selectedCust.address&&(
          <div style={{ background:"#fff",borderRadius:16,padding:"16px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ fontSize:14,color:"#374151",lineHeight:1.8 }}>
              ...
              {selectedCust.address.refs&&<><br/><span style={{ fontSize:12,color:"#9CA3AF" }}>📌 {selectedCust.address.refs}</span></>}
            </div>
          </div>
        )}

        <div style={{ background:"#fff",borderRadius:16,padding:"16px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize:13,fontWeight:800,color:"#111",marginBottom:12 }}>📦 Historial de pedidos</div>
          {co.length===0?(
            <div style={{ textAlign:"center",padding:"14px 0",color:"#9CA3AF",fontSize:13 }}>Sin pedidos registrados</div>
          ):co.slice(0,8).map((o,i)=>(
            <div key={o.id||i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:i<Math.min(co.length,8)-1?"1px solid #F9FAFB":"none" }}>
              ...
              <div style={{ textAlign:"right" }}>
                <div style={{ fontSize:13,fontWeight:800,color:"#FF4D4D" }}>${o.total||0}</div>
                <div style={{ background:`${STATUS_COLOR[o.status]||"#6B7280"}18`,color:STATUS_COLOR[o.status]||"#6B7280",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99 }}>{STATUS_LABEL[o.status]||o.status}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"flex",gap:10 }}>
          <button onClick={()=>toggleActive(selectedCust.id)} style={{ flex:1,padding:"12px",background:selectedCust.active!==false?"#FFF5F5":"#F0FDF4",color:selectedCust.active!==false?"#FF4D4D":"#16A34A",border:`1.5px solid ${selectedCust.active!==false?"#FFE4E4":"#BBF7D0"}`,borderRadius:12,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit" }}>
            {selectedCust.active!==false?"🔒 Desactivar":"✅ Activar"}
          </button>
          <button onClick={()=>deleteCustomer(selectedCust.id)} style={{ flex:1,padding:"12px",background:"#FFF5F5",color:"#FF4D4D",border:"1.5px solid #FFE4E4",borderRadius:12,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit" }}>
            🗑️ Eliminar
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ padding:"16px 24px",display:"flex",flexDirection:"column",gap:12,paddingBottom:100 }}>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:9 }}>
        {[{icon:"👥",l:"Total",v:allCustomers.length,bg:"#EEF2FF",c:"#6366F1"},{icon:"🆕",l:"Este mes",v:allCustomers.filter(c=>{const d=new Date(c.createdAt);return d.getMonth()===thisMonth&&d.getFullYear()===thisYear;}).length,bg:"#F0FDF4",c:"#16A34A"},{icon:"🟢",l:"Activos",v:allCustomers.filter(c=>c.active!==false).length,bg:"#FFF7ED",c:"#F97316"}].map((s,i)=>(
          <div key={i} style={{ background:s.bg,borderRadius:13,padding:"12px 10px",textAlign:"center" }}><div style={{ fontSize:20 }}>{s.icon}</div><div style={{ fontSize:18,fontWeight:900,color:s.c,marginTop:3 }}>{s.v}</div><div style={{ fontSize:10,color:"#6B7280",marginTop:1 }}>{s.l}</div></div>
        ))}
      </div>
      ...
      {filtered.length===0&&(
        <div style={{ textAlign:"center",padding:"44px 0",color:"#9CA3AF" }}>
          <div style={{ fontSize:48 }}>👥</div>
          <div style={{ fontSize:15,fontWeight:700,color:"#374151",marginTop:10 }}>{allCustomers.length===0?"Sin clientes registrados":"Sin resultados"}</div>
          {allCustomers.length===0&&<div style={{ fontSize:13,color:"#9CA3AF",marginTop:6 }}>Los clientes aparecerán aquí cuando se registren en la app</div>}
        </div>
      )}
      {filtered.map(c=>(
        <div key={c.id} onClick={()=>setSelectedCust(c)} style={{ background:"#fff",borderRadius:18,padding:"14px 16px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)",cursor:"pointer",display:"flex",alignItems:"center",gap:12,opacity:c.active===false?0.55:1 }}>
          <div style={{ width:46,height:46,borderRadius:99,background:c.active===false?"#F3F4F6":"linear-gradient(135deg,#FF4D4D,#FF6B6B)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:800,color:"#fff",flexShrink:0 }}>
          ...
          <div style={{ flex:1,minWidth:0 }}>
            ...
            <div style={{ fontSize:11,color:"#9CA3AF",marginTop:1 }}>{c.address?.colonia?`📍 ${c.address.colonia} · `:""}Desde {new Date(c.createdAt).toLocaleDateString("es-MX",{day:"numeric",month:"short",year:"numeric"})}</div>
          </div>
          <div style={{ flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4 }}>
            ...
            <svg width="7" height="12" viewBox="0 0 8 14" fill="none" style={{ opacity:0.25 }}><path d="M1 1l6 6-6 6" stroke="#111" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          </div>
        </div>
      ))}
    </div>
...
// ADMIN SCREEN — 4 tabs
// ─────────────────────────────────────────
function AdminScreen({ restaurants, menuItems, setRestaurants, setMenuItems, showToast, onDataChange }) {
  // ── Nav tab (top-level) ──
  ...
  // ── Load orders — localStorage first, then try DB ──
  const loadOrders = () => {
    const local = loadLocalOrders();
    if(local.length > 0) {
      setOrders(local);
    ...
    sbGet("orders","order=created_at.desc&limit=100")
      .then(d=>{ if(Array.isArray(d)&&d.length>0){ setOrders(d); saveLocalOrders(d); } setOrdersLoading(false); })
  ...
  // Auto-refresh every 15s when on pedidos/estadisticas tab
  useEffect(()=>{
    if(adminTab!=="pedidos"&&adminTab!=="estadisticas") return;
  ...
  // ── LOCAL STORAGE for restaurants & menu (always works, no DB needed) ──
  const LOCAL_RESTS_KEY  = "yg_local_restaurants";
  const LOCAL_ITEMS_KEY  = "yg_local_menu_items";
  const loadLocalRests   = () => { try { return JSON.parse(localStorage.getItem(LOCAL_RESTS_KEY)||"null"); } catch { return null; } };
  const saveLocalRests   = (r) => localStorage.setItem(LOCAL_RESTS_KEY, JSON.stringify(r));
  const loadLocalItems   = () => { try { return JSON.parse(localStorage.getItem(LOCAL_ITEMS_KEY)||"null"); } catch { return null; } };
  const saveLocalItems   = (r) => localStorage.setItem(LOCAL_ITEMS_KEY, JSON.stringify(r));

  const nextId = () => Date.now() + Math.floor(Math.random()*1000);

  const addRestaurant = async (f) => {
    const newRest = { ...f, id: nextId(), active: f.active!==false, rating: f.rating||4.5, createdAt: new Date().toISOString() };
    const updated = [...restaurants, newRest];
    setRestaurants(updated); saveLocalRests(updated);
    showToast("Restaurante agregado ✓");
    onDataChange?.(); bumpMenuSync();
    sbPost("restaurants", f).catch(()=>{});
  };

  const updateRestaurant = async (f) => {
    const updated = restaurants.map(r => r.id===activeRest.id ? {...r,...f} : r);
    setRestaurants(updated); saveLocalRests(updated);
    setActiveRest({...activeRest,...f});
    showToast("Guardado ✓");
    onDataChange?.(); bumpMenuSync();
    sbPatch("restaurants", activeRest.id, f).catch(()=>{});
  };

  const deleteRestaurant = async (id) => {
    const updR = restaurants.filter(r=>r.id!==id);
    const updI = menuItems.filter(i=>i.restaurant_id!==id);
    setRestaurants(updR); setMenuItems(updI);
    saveLocalRests(updR); saveLocalItems(updI);
    setView("list"); setActiveRest(null);
    showToast("Eliminado");
    onDataChange?.(); bumpMenuSync();
    sbDel("restaurants", id).catch(()=>{});
  };

  const toggleRest = async (r) => {
    const updated = restaurants.map(x => x.id===r.id ? {...x,active:!x.active} : x);
    setRestaurants(updated); saveLocalRests(updated);
    if(activeRest?.id===r.id) setActiveRest(p=>({...p,active:!p.active}));
    onDataChange?.(); bumpMenuSync();
    sbPatch("restaurants", r.id, {active:!r.active}).catch(()=>{});
  };

  const addItem = async (f) => {
    const newItem = { ...f, id: nextId(), restaurant_id: activeRest.id, available: f.available!==false, createdAt: new Date().toISOString() };
    const updated = [...menuItems, newItem];
    setMenuItems(updated); saveLocalItems(updated);
    showToast("Platillo agregado ✓");
    onDataChange?.(); bumpMenuSync();
    sbPost("menu_items", {...f, restaurant_id: activeRest.id}).catch(()=>{});
  };

  const updateItem = async (f) => {
    const updated = menuItems.map(i => i.id===editTarget.id ? {...i,...f} : i);
    setMenuItems(updated); saveLocalItems(updated);
    showToast("Guardado ✓");
    onDataChange?.(); bumpMenuSync();
    sbPatch("menu_items", editTarget.id, f).catch(()=>{});
  };
  ...
  const deleteItem = async (id) => {
    ...
    onDataChange?.(); bumpMenuSync();
    sbDel("menu_items", id).catch(()=>{});
  };

  const toggleItem = async (item) => {
    const updated = menuItems.map(i => i.id===item.id ? {...i,available:!i.available} : i);
    setMenuItems(updated); saveLocalItems(updated);
    onDataChange?.(); bumpMenuSync();
    sbPatch("menu_items", item.id, {available:!item.available}).catch(()=>{});
  };
  ...
  // Refresh accounts whenever restTab changes to "cuentas"
  useEffect(()=>{
    if(restTab==="cuentas") {
      setRestAccounts(loadRestaurantAccounts());
    }
  },[restTab]);
  ...
  const saveRestAccount = () => {
    ...
    const payload = {
      id: editRestAccount ? editRestAccount.id : Date.now(),
      restaurantName: restAccountForm.restaurantName.trim(),
      restaurantId: restAccountForm.restaurantId.trim() || restAccountForm.restaurantName.trim().toLowerCase().replace(/\s+/g,"-"),
      emoji: restAccountForm.emoji || "🍽️",
      user: restAccountForm.user.trim().toLowerCase(), // always lowercase
      pass: restAccountForm.pass,
      phone: restAccountForm.phone.trim(),
      active: true, // always active when created/edited
      createdAt: editRestAccount?.createdAt || new Date().toISOString().split("T")[0],
    };

    // Read fresh from localStorage (avoid stale state)
    const current = loadRestaurantAccounts();
    let updated;
    if(editRestAccount) {
      updated = current.map(a => a.id===editRestAccount.id ? payload : a);
    } else {
      // Remove any duplicate with same user
      const deduped = current.filter(a => a.user !== payload.user);
      updated = [...deduped, payload];
    }

    // Save to localStorage
    saveRestaurantAccounts(updated);
    // Update React state
    setRestAccounts(updated);
    setRestAccountModal(false);
    setEditRestAccount(null);

    // Verify it saved
    const verify = loadRestaurantAccounts();
    if(verify.find(a => a.user === payload.user)) {
      showToast(`✓ Cuenta guardada: ${payload.user}`);
    } else {
  ...
  const STATUS_COLOR = { pending:"#F59E0B",confirmed:"#6366F1",delivering:"#2563EB",delivered:"#16A34A",cancelled:"#EF4444" };
  const STATUS_LABEL = { pending:"Pendiente",confirmed:"Confirmado",delivering:"En camino",delivered:"Entregado",cancelled:"Cancelado" };
  const updateOrderStatus = async (order, ns) => {
    // Update local state immediately
    const updated = orders.map(o=>o.id===order.id?{...o,status:ns}:o);
    setOrders(updated);
    if(selectedOrder?.id===order.id) setSelectedOrder({...order,status:ns});
    // Persist locally
    saveLocalOrders(updated);
    showToast(`Estado: ${STATUS_LABEL[ns]}`);
    // Try DB
    sbPatch("orders",order.id,{status:ns}).catch(()=>{});
  };
    ...
    <div style={{ paddingBottom:80 }}>
      ...
      <div style={{ background:"#fff",padding:"14px 24px 0",borderBottom:"1px solid #F3F4F6",position:"sticky",top:0,zIndex:90 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
          ...
          {adminTab==="pedidos"&&(
            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
              <div style={{ background:"#EEF2FF",borderRadius:10,padding:"4px 10px",fontSize:12,fontWeight:700,color:"#6366F1" }}>{totalPending} activos</div>
              <div onClick={loadOrders} style={{ width:32,height:32,borderRadius:10,background:"#F9FAFB",border:"1.5px solid #F3F4F6",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14 }}>🔄</div>
            </div>
          )}
        </div>
        {/* Nav tabs */}
                  ...
                  {Object.entries(grouped).map(([cat,items])=>(
                    <div key={cat}>
                      ...
                      <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
                        {items.map(item=>(
                          <div key={item.id} style={{ background:"#fff",borderRadius:14,padding:"12px 14px",boxShadow:"0 1px 6px rgba(0,0,0,0.05)",opacity:item.available?1:0.5,display:"flex",alignItems:"flex-start",gap:10 }}>
                            <div style={{ flex:1 }}>
                              <div style={{ display:"flex",alignItems:"center",gap:7,flexWrap:"wrap",marginBottom:3 }}>
                                {/* Type badge */}
                                {item.itemType==="drink"&&<span style={{ background:"#EEF2FF",color:"#6366F1",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99 }}>🥤 Bebida</span>}
                                {item.itemType==="combo"&&<span style={{ background:"#FFF7ED",color:"#F97316",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99 }}>🎁 Combo</span>}
                                {item.itemType==="promo"&&<span style={{ background:"#FFF5F5",color:"#FF4D4D",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99 }}>🏷️ Promo</span>}
                                {!item.available&&<span style={{ background:"#FEE2E2",color:"#EF4444",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99 }}>Agotado</span>}
                                {item.promoType==="2x1"&&<span style={{ background:"#FF4D4D",color:"#fff",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:99 }}>2×1</span>}
                                {item.promoType==="descuento"&&item.discountPct&&<span style={{ background:"#F97316",color:"#fff",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:99 }}>{item.discountPct}% OFF</span>}
                              </div>
                              <div style={{ fontSize:14,fontWeight:700,color:"#111" }}>{item.name}</div>
                              {item.size&&<div style={{ fontSize:11,color:"#9CA3AF",marginTop:1 }}>📐 {item.size}</div>}
                              {item.description&&<div style={{ fontSize:12,color:"#9CA3AF",marginTop:2,lineHeight:1.5 }}>{item.description}</div>}
                              {item.comboItems&&<div style={{ fontSize:11,color:"#F97316",marginTop:3,lineHeight:1.5,whiteSpace:"pre-line" }}>{item.comboItems}</div>}
                              {item.extras&&<div style={{ fontSize:11,color:"#6366F1",marginTop:3 }}>⚙️ {item.extras}</div>}
                              <div style={{ display:"flex",alignItems:"center",gap:8,marginTop:5 }}>
                                <span style={{ fontSize:15,fontWeight:900,color:"#FF4D4D" }}>${item.price}</span>
                                {item.preparationTime&&<span style={{ fontSize:11,color:"#9CA3AF" }}>🕐 {item.preparationTime}</span>}
                                {item.calories&&<span style={{ fontSize:11,color:"#9CA3AF" }}>🔥 {item.calories} kcal</span>}
                                {item.isSpicy&&<span style={{ fontSize:12 }}>🌶️</span>}
                                {item.isVegan&&<span style={{ fontSize:12 }}>🌱</span>}
                                {item.isGlutenFree&&<span style={{ fontSize:12 }}>🌾</span>}
                              </div>
                            </div>
      ...
      {/* ── TAB: CLIENTES ── */}
      {adminTab==="clientes"&&(
        <AdminClientesTab orders={orders}/>
      )}
      ...
      {adminTab==="config"&&(
        <div style={{ padding:"16px 24px 80px",display:"flex",flexDirection:"column",gap:13 }}>
          ...
          {/* Diagnostic — All restaurant accounts in localStorage */}
          {(()=>{
            const accounts = loadRestaurantAccounts();
            return (
              <div style={{ background:"#FFF7ED",borderRadius:16,padding:"16px 18px",border:"1.5px solid #FED7AA" }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
                  <div style={{ fontSize:13,fontWeight:800,color:"#F97316" }}>🍽️ Cuentas de restaurante guardadas ({accounts.length})</div>
                  <div onClick={()=>setRestAccounts(loadRestaurantAccounts())} style={{ background:"#F97316",color:"#fff",borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:700,cursor:"pointer" }}>🔄 Actualizar</div>
                </div>
                {accounts.length===0?(
                  <div style={{ fontSize:12,color:"#92400E",background:"#FEF3C7",borderRadius:10,padding:"10px 12px" }}>
                    ⚠️ No hay cuentas guardadas en localStorage. Ve a Restaurantes → 🔑 Cuentas → ＋ Cuenta para crear una.
                  </div>
                ):accounts.map((a,i)=>(
                  <div key={a.id||i} style={{ padding:"10px 0",borderBottom:i<accounts.length-1?"1px solid #FED7AA":"none" }}>
                    <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:4 }}>
                      <span style={{ fontSize:16 }}>{a.emoji||"🍽️"}</span>
                      <span style={{ fontSize:13,fontWeight:800,color:"#111" }}>{a.restaurantName}</span>
                      <span style={{ background:a.active?"#ECFDF5":"#FFF5F5",color:a.active?"#16A34A":"#FF4D4D",fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:99 }}>{a.active?"Activa":"Inactiva"}</span>
                    </div>
                    <div style={{ display:"flex",justifyContent:"space-between",marginBottom:2 }}>
                      <span style={{ fontSize:11,color:"#6B7280" }}>Usuario</span>
                      <span style={{ fontSize:11,fontWeight:700,fontFamily:"monospace",color:"#111" }}>{a.user}</span>
                    </div>
                    <div style={{ display:"flex",justifyContent:"space-between" }}>
                      <span style={{ fontSize:11,color:"#6B7280" }}>Contraseña</span>
                      <span style={{ fontSize:11,fontWeight:700,fontFamily:"monospace",color:"#111" }}>{a.pass}</span>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {modal==="addItem"&&<Modal title="➕ Agregar al menú" onClose={()=>setModal(null)}><ItemForm onSave={addItem} onClose={()=>setModal(null)}/></Modal>}
      {modal==="editItem"&&<Modal title="✏️ Editar artículo" onClose={()=>setModal(null)}><ItemForm initial={editTarget} onSave={updateItem} onClose={()=>setModal(null)}/></Modal>}
...
function RestForm({ initial, onSave, onClose }) {
  const [form,setForm]=useState(initial||{name:"",cuisine:"",emoji:"🍽️",address:"",phone:"",hours:"",active:true,restUser:"",restPass:""});
  const [saving,setSaving]=useState(false);
  ...
  const handleSave = async () => {
    if(!form.name) return;
    if(form.restPass && form.restPass.length < 6){ setPassErr("Mínimo 6 caracteres"); return; }
    if(form.restUser && !form.restUser.includes("@")){ setPassErr("Correo inválido"); return; }
    setPassErr("");
    setSaving(true);

    // If credentials provided, create/update restaurant account automatically
    if(form.restUser && form.restPass) {
      const accounts = loadRestaurantAccounts();
      const existing = accounts.find(a=>a.restaurantName===form.name);
      const payload  = { restaurantName:form.name, emoji:form.emoji||"🍽️", user:form.restUser.toLowerCase().trim(), pass:form.restPass, phone:form.phone||"", active:true, restaurantId:form.name.toLowerCase().replace(/\s+/g,"-"), createdAt:new Date().toISOString().split("T")[0] };
      let updated;
      if(existing) { updated = accounts.map(a=>a.restaurantName===form.name?{...a,...payload}:a); }
      else         { updated = [...accounts, {id:Date.now(),...payload}]; }
      saveRestaurantAccounts(updated);
    }
  ...
  return <>
    <div style={{ marginBottom:14 }}>
      ...
      <div style={{ display:"flex",flexWrap:"wrap",gap:8 }}>
        {EMOJIS.map(e=><div key={e} onClick={()=>set("emoji")(e)} style={{ width:42,height:42,borderRadius:12,fontSize:22,display:"flex",alignItems:"center",justifyContent:"center",background:form.emoji===e?"#F97316":"#F9FAFB",border:`1.5px solid ${form.emoji===e?"#F97316":"#F3F4F6"}`,cursor:"pointer" }}>{e}</div>)}
      </div>
    </div>
    <Field label="NOMBRE DEL RESTAURANTE" value={form.name} onChange={set("name")} placeholder="Ej: Nápoli Pizza"/>
    ...
    <Field label="TELÉFONO" value={form.phone} onChange={set("phone")} placeholder="473 000 0000"/>
    <Field label="HORARIO" value={form.hours} onChange={set("hours")} placeholder="12:00 – 23:00"/>

    {/* Credentials section */}
    <div style={{ background:"#FFF7ED",borderRadius:14,padding:"14px 16px",marginBottom:14,border:"1.5px solid #FED7AA" }}>
      <div style={{ fontSize:12,fontWeight:800,color:"#F97316",marginBottom:12,letterSpacing:0.4 }}>🔑 CREDENCIALES DE ACCESO AL PANEL</div>
      <div style={{ marginBottom:10 }}>
        <div style={{ fontSize:11,fontWeight:700,color:"#6B7280",marginBottom:5 }}>USUARIO (correo del restaurante)</div>
        <input value={form.restUser||""} onChange={e=>set("restUser")(e.target.value)} placeholder="napoli@yummigo.mx" autoCapitalize="none"
          style={{ width:"100%",padding:"10px 13px",background:"#fff",border:"2px solid #FDBA74",borderRadius:11,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
          onFocus={e=>e.target.style.borderColor="#F97316"} onBlur={e=>e.target.style.borderColor="#FDBA74"}/>
      </div>
      <div>
        <div style={{ fontSize:11,fontWeight:700,color:"#6B7280",marginBottom:5 }}>CONTRASEÑA (mínimo 6 caracteres)</div>
        <input value={form.restPass||""} onChange={e=>{set("restPass")(e.target.value);setPassErr("");}} placeholder="Contraseña segura" type="password"
          style={{ width:"100%",padding:"10px 13px",background:"#fff",border:`2px solid ${passErr?"#FF4D4D":"#FDBA74"}`,borderRadius:11,fontSize:14,fontFamily:"monospace",color:"#111",outline:"none",boxSizing:"border-box" }}
          onFocus={e=>e.target.style.borderColor="#F97316"} onBlur={e=>e.target.style.borderColor=passErr?"#FF4D4D":"#FDBA74"}/>
        {passErr&&<div style={{ fontSize:11,color:"#FF4D4D",marginTop:3 }}>⚠️ {passErr}</div>}
      </div>
      <div style={{ fontSize:11,color:"#92400E",marginTop:8,lineHeight:1.5 }}>
        💡 Con estas credenciales el restaurante podrá iniciar sesión en su <strong>Panel de Restaurante</strong> para gestionar pedidos en tiempo real.
      </div>
    </div>

    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
      <span style={{ fontSize:14,fontWeight:700,color:"#111" }}>Restaurante activo</span>
      <Toggle value={form.active} onChange={()=>set("active")(!form.active)}/>
    </div>
    <button onClick={handleSave} style={{ width:"100%",padding:"15px",background:saving?"#9CA3AF":"#F97316",color:"#fff",border:"none",borderRadius:14,fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:saving?"none":"0 4px 14px rgba(249,115,22,0.3)" }}>
...
function ItemForm({ initial, onSave, onClose }) {
  const [form, setForm] = useState(initial || {
    name:"", description:"", price:"", category:"Platos principales",
    available:true,
    // Extended fields
    itemType:"food",          // food | drink | combo | promo
    promoType:"",             // 2x1 | descuento | precio_especial | ""
    discountPct:"",           // % descuento
    comboItems:"",            // descripción de qué incluye el combo
    size:"",                  // tamaño (S/M/L, 500ml, etc.)
    preparationTime:"",       // tiempo estimado
    calories:"",
    isSpicy:false,
    isVegan:false,
    isGlutenFree:false,
    extras:"",                // opciones extra (separadas por coma)
  });
  const [saving, setSaving] = useState(false);
  const set = k => v => setForm(f=>({...f,[k]:v}));

  const ITEM_TYPES = [
    { id:"food",  icon:"🍽️", label:"Platillo" },
    { id:"drink", icon:"🥤", label:"Bebida"   },
    { id:"combo", icon:"🎁", label:"Combo"    },
    { id:"promo", icon:"🏷️", label:"Promo"   },
  ];

  const FOOD_CATS  = ["Platos principales","Entradas","Ensaladas","Sopas","Tacos","Pizzas","Burgers","Rolls","Postres","Otros"];
  const DRINK_CATS = ["Refrescos","Aguas frescas","Jugos","Cafés","Cervezas","Cocteles","Licuados","Tés","Sin alcohol","Bebidas"];
  const COMBO_CATS = ["Combos","Menú del día","Paquetes"];
  const PROMO_CATS = ["Promociones","Ofertas","2x1"];

  const categoriesByType = { food:FOOD_CATS, drink:DRINK_CATS, combo:COMBO_CATS, promo:PROMO_CATS };
  const currentCats = categoriesByType[form.itemType] || FOOD_CATS;

  const PROMO_TYPES = [
    { id:"2x1",           label:"2×1 — Lleva 2 paga 1",       color:"#FF4D4D" },
    { id:"descuento",     label:"% de descuento",               color:"#F97316" },
    { id:"precio_esp",    label:"Precio especial",              color:"#6366F1" },
    { id:"gratis_item",   label:"Producto gratis al comprar X", color:"#16A34A" },
  ];

  const Tags = ({label,k}) => (
    <div onClick={()=>set(k)(!form[k])}
      style={{ display:"flex",alignItems:"center",gap:8,padding:"10px 14px",borderRadius:12,border:`1.5px solid ${form[k]?"#FF4D4D":"#F3F4F6"}`,background:form[k]?"#FFF5F5":"#F9FAFB",cursor:"pointer",flex:1 }}>
      <div style={{ width:18,height:18,borderRadius:99,border:`2px solid ${form[k]?"#FF4D4D":"#D1D5DB"}`,background:form[k]?"#FF4D4D":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
        {form[k]&&<span style={{ color:"#fff",fontSize:11,fontWeight:800 }}>✓</span>}
      </div>
      <span style={{ fontSize:13,fontWeight:600,color:form[k]?"#FF4D4D":"#374151" }}>{label}</span>
    </div>
  );
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      category: form.category || currentCats[0],
    };
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      {/* Item type selector */}
      <div>
        <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:8 }}>TIPO DE ARTÍCULO</div>
        <div style={{ display:"flex",gap:8 }}>
          {ITEM_TYPES.map(t=>(
            <div key={t.id} onClick={()=>{ set("itemType")(t.id); set("category")(categoriesByType[t.id][0]); set("promoType")(""); }}
              style={{ flex:1,padding:"10px 4px",borderRadius:13,border:`2px solid ${form.itemType===t.id?"#FF4D4D":"#F3F4F6"}`,background:form.itemType===t.id?"#FFF5F5":"#F9FAFB",cursor:"pointer",textAlign:"center" }}>
              <div style={{ fontSize:22,marginBottom:3 }}>{t.icon}</div>
              <div style={{ fontSize:11,fontWeight:700,color:form.itemType===t.id?"#FF4D4D":"#6B7280" }}>{t.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>
          {form.itemType==="drink"?"NOMBRE DE LA BEBIDA *":form.itemType==="combo"?"NOMBRE DEL COMBO *":form.itemType==="promo"?"NOMBRE DE LA PROMO *":"NOMBRE DEL PLATILLO *"}
        </div>
        <input value={form.name} onChange={e=>set("name")(e.target.value)}
          placeholder={form.itemType==="drink"?"Ej: Agua de Jamaica 500ml":form.itemType==="combo"?"Ej: Combo Familiar #1":form.itemType==="promo"?"Ej: 2×1 en Pizzas los Martes":"Ej: Pizza Margherita"}
          style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:15,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
          onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
      </div>

      {/* Description */}
      <div>
        <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>DESCRIPCIÓN</div>
        <textarea value={form.description} onChange={e=>set("description")(e.target.value)}
          placeholder={form.itemType==="combo"?"Ej: 2 hamburguesas, papas medianas y 2 refrescos":form.itemType==="promo"?"Ej: Válido lunes y martes. Aplica en pizzas personales.":"Ej: Con jitomate, mozzarella fresca y albahaca"}
          rows={3}
          style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box",resize:"none",lineHeight:1.6 }}
          onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
      </div>
      ...
      <div style={{ display:"flex",gap:10 }}>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>PRECIO ($) *</div>
          <div style={{ position:"relative" }}>
            <span style={{ position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",fontSize:15,fontWeight:700,color:"#374151" }}>$</span>
            <input type="number" value={form.price} onChange={e=>set("price")(e.target.value)} placeholder="0.00" min="0"
              style={{ width:"100%",padding:"12px 12px 12px 28px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:16,fontWeight:700,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
              onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
          </div>
        </div>
        {form.itemType==="drink"&&(
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>TAMAÑO / PRESENTACIÓN</div>
            <input value={form.size} onChange={e=>set("size")(e.target.value)} placeholder="Ej: 500ml, Grande, 1L"
              style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
              onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
          </div>
        )}
        {(form.itemType==="food"||form.itemType==="combo")&&(
          <div style={{ flex:1 }}>
            <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>TIEMPO APROX.</div>
            <input value={form.preparationTime} onChange={e=>set("preparationTime")(e.target.value)} placeholder="Ej: 15 min"
              style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
              onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
          </div>
        )}
      ...
      {/* Promo type (only for promo items) */}
      {form.itemType==="promo"&&(
        <div>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:8 }}>TIPO DE PROMOCIÓN</div>
          <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
            {PROMO_TYPES.map(pt=>(
              <div key={pt.id} onClick={()=>set("promoType")(pt.id)}
                style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:12,border:`2px solid ${form.promoType===pt.id?pt.color:"#F3F4F6"}`,background:form.promoType===pt.id?`${pt.color}10`:"#F9FAFB",cursor:"pointer" }}>
                <div style={{ width:20,height:20,borderRadius:99,border:`2px solid ${form.promoType===pt.id?pt.color:"#D1D5DB"}`,background:form.promoType===pt.id?pt.color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                  {form.promoType===pt.id&&<span style={{ color:"#fff",fontSize:10,fontWeight:900 }}>✓</span>}
                </div>
                <span style={{ fontSize:14,fontWeight:600,color:form.promoType===pt.id?pt.color:"#374151" }}>{pt.label}</span>
              </div>
            ))}
          </div>
          {form.promoType==="descuento"&&(
            <div style={{ marginTop:10 }}>
              <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>PORCENTAJE DE DESCUENTO (%)</div>
              <div style={{ position:"relative" }}>
                <input type="number" value={form.discountPct} onChange={e=>set("discountPct")(e.target.value)} placeholder="Ej: 20" min="1" max="99"
                  style={{ width:"100%",padding:"12px 44px 12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:15,fontWeight:700,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
                  onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
                <span style={{ position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",fontSize:16,fontWeight:700,color:"#9CA3AF" }}>%</span>
              </div>
            </div>
          )}
        </div>
      ...
      {form.itemType==="combo"&&(
        <div>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>¿QUÉ INCLUYE EL COMBO?</div>
          <textarea value={form.comboItems} onChange={e=>set("comboItems")(e.target.value)}
            placeholder={"Ej:\n• 2 hamburguesas clásicas\n• 1 orden de papas grandes\n• 2 refrescos 600ml"}
            rows={4}
            style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box",resize:"none",lineHeight:1.7 }}
            onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
        </div>
      )}
          ...
          {currentCats.map(c=>(
            <div key={c} onClick={()=>set("category")(c)}
              style={{ padding:"7px 14px",borderRadius:99,fontSize:12,fontWeight:700,cursor:"pointer",
                background:form.category===c?"#FF4D4D":"#F9FAFB",
                color:form.category===c?"#fff":"#6B7280",
                border:`1.5px solid ${form.category===c?"#FF4D4D":"#F3F4F6"}` }}>
              {c}
            </div>
          ))}
        </div>
      ...
      {(form.itemType==="food"||form.itemType==="combo")&&(
        <div>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>OPCIONES / EXTRAS (separadas por coma)</div>
          <input value={form.extras} onChange={e=>set("extras")(e.target.value)}
            placeholder="Ej: Sin cebolla, Extra queso +$15, Salsa picante"
            style={{ width:"100%",padding:"12px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
            onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
          <div style={{ fontSize:11,color:"#9CA3AF",marginTop:4 }}>El cliente podrá elegir estas opciones al ordenar</div>
        </div>
      )}
      ...
      {form.itemType==="food"&&(
        <div>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:8 }}>ETIQUETAS</div>
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
            <Tags label="🌶️ Picante" k="isSpicy"/>
            <Tags label="🌱 Vegano"  k="isVegan"/>
            <Tags label="🌾 Sin gluten" k="isGlutenFree"/>
          </div>
        </div>
      )}

      {/* Calories */}
      {(form.itemType==="food"||form.itemType==="drink")&&(
        <div>
          <div style={{ fontSize:12,fontWeight:700,color:"#6B7280",marginBottom:6 }}>CALORÍAS (opcional)</div>
          <input type="number" value={form.calories} onChange={e=>set("calories")(e.target.value)} placeholder="Ej: 350 kcal"
            style={{ width:"100%",padding:"11px 14px",background:"#fff",border:"2px solid #F3F4F6",borderRadius:12,fontSize:14,fontFamily:"inherit",color:"#111",outline:"none",boxSizing:"border-box" }}
            onFocus={e=>e.target.style.borderColor="#FF4D4D"} onBlur={e=>e.target.style.borderColor="#F3F4F6"}/>
        </div>
      )}

      {/* Available toggle */}
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 16px",background:"#F9FAFB",borderRadius:12,border:"1.5px solid #F3F4F6" }}>
        <div>
          <div style={{ fontSize:14,fontWeight:700,color:"#111" }}>Disponible ahora</div>
          <div style={{ fontSize:12,color:"#9CA3AF",marginTop:1 }}>Los clientes pueden ordenarlo</div>
        </div>
        <Toggle value={form.available} onChange={()=>set("available")(!form.available)}/>
      </div>

      <button onClick={handleSave} disabled={!form.name||!form.price||saving}
        style={{ width:"100%",padding:"15px",background:(!form.name||!form.price||saving)?"#E5E7EB":"#FF4D4D",color:(!form.name||!form.price||saving)?"#9CA3AF":"#fff",border:"none",borderRadius:14,fontSize:15,fontWeight:700,cursor:(!form.name||!form.price||saving)?"not-allowed":"pointer",fontFamily:"inherit",boxShadow:(!form.name||!form.price||saving)?"none":"0 4px 14px rgba(255,77,77,0.3)" }}>
        {saving?"Guardando…":initial?"Guardar cambios ✓":"Agregar al menú ✓"}
      </button>
    </div>
  );
}
...
function RestaurantPanelScreen({ onBack, session, onLogout }) {
  ...
  const loadOrders = () => {
    const myName = (session.restaurantName||"").toLowerCase().trim();
    const myId   = String(session.restaurantId||"");

    // 1. From localStorage
    const all = loadLocalOrders();
    const localMatches = all.filter(o => {
      const oName = (o.restaurant_name||"").toLowerCase().trim();
      const oId   = String(o.restaurant_id||"");
      return oName === myName || oId === myId ||
             (myName.length > 3 && oName.includes(myName)) ||
             (myName.length > 3 && myName.includes(oName));
    }).sort((a,b) => new Date(b.created_at) - new Date(a.created_at));

    setOrders(localMatches);
    setLoading(false);

    // 2. Also pull from Supabase and merge
    sbGet("orders", `order=created_at.desc&limit=100`)
      .then(d => {
        if(!Array.isArray(d) || d.length === 0) return;
        const remoteMatches = d.filter(o => {
          const oName = (o.restaurant_name||"").toLowerCase().trim();
          const oId   = String(o.restaurant_id||"");
          return oName === myName || oId === myId ||
                 (myName.length > 3 && oName.includes(myName));
        ...
        if(remoteMatches.length > 0) {
          // Merge: remote overrides local for same IDs
          const localMap = new Map(localMatches.map(o=>[String(o.id),o]));
          remoteMatches.forEach(ro => localMap.set(String(ro.id), {...(localMap.get(String(ro.id))||{}), ...ro}));
          const merged = Array.from(localMap.values()).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
          saveLocalOrders([...loadLocalOrders().filter(o=>!remoteMatches.find(r=>String(r.id)===String(o.id))), ...merged]);
          setOrders(merged);
        }
      }).catch(()=>{});
  };
  ...
  // Live sync every 3s — catches orders from any device
  useLiveSync(loadOrders, 3000);
  ...
  const updateStatus = async (order, newStatus, extra={}) => {
    ...
    // Update localStorage + broadcast immediately
    updateLocalOrder(order.id, patch);
    bumpSync();
    // Update React state
    setOrders(p => p.map(o => o.id===order.id ? {...o,...patch} : o));
    if(selectedOrder?.id===order.id) setSelectedOrder(s=>({...s,...patch}));
    // Push to Supabase so other devices see it
    sbPatch("orders", order.id, patch).catch(()=>{});
  };
...
// ─────────────────────────────────────────
function ProfileScreen({ showToast, restaurants, menuItems, setRestaurants, setMenuItems, customerSession, onCustomerLogout, onShowAuthGate, onDataChange }) {
  ...
  if(view==="restaurant") {
    ...
    if(!ok) return (
      <LoginScreen role="restaurant" onBack={()=>setView("main")}
        accounts={loadRestaurantAccounts()}
    ...
    return (
      <div>
        <div style={{ background:"#fff",padding:"10px 24px 10px",borderBottom:"1px solid #F3F4F6",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <button onClick={()=>setView("main")} style={{ background:"none",border:"none",fontSize:13,fontWeight:700,color:"#6B7280",cursor:"pointer" }}>← Volver</button>
          <div style={{ display:"flex",alignItems:"center",gap:6 }}>
            <span style={{ fontSize:16 }}>{session.restaurantEmoji||"🍽️"}</span>
            <span style={{ fontSize:13,fontWeight:700,color:"#F97316" }}>{session.restaurantName||"Restaurante"}</span>
          </div>
  ...
  if(view==="admin") {
    ...
    return (
      <div>
        ...
        <AdminScreen restaurants={restaurants||[]} menuItems={menuItems||[]} setRestaurants={setRestaurants||(() => {})} setMenuItems={setMenuItems||(() => {})} showToast={showToast||(() => {})} onDataChange={onDataChange}/>
      </div>
    );
  }
...
function App() {
  ...
  // Load restaurants and menu items — always reads localStorage first
  const loadAppData = () => {
    const localRests = (() => { try { return JSON.parse(localStorage.getItem("yg_local_restaurants")||"null"); } catch { return null; } })();
    const localItems = (() => { try { return JSON.parse(localStorage.getItem("yg_local_menu_items")||"null"); } catch { return null; } })();
    if(localRests && localRests.length > 0) {
      setRestaurants(localRests.filter(r=>r.active!==false));
      setMenuItems(localItems && localItems.length > 0 ? localItems : DEMO_MENU_ITEMS);
      setLoading(false);
      return true;
    }
    return false;
  ...
  // Auto-refresh menu when any device/tab updates it
  useMenuSync(loadAppData);

  useEffect(()=>{
    // 1. Load local data immediately (admin-added restaurants)
    if(loadAppData()) return;
    // 2. Try remote DB
    Promise.all([
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Helvetica Neue',sans-serif",background:"#F9FAFB",height:"100vh",maxWidth:430,margin:"0 auto",position:"relative",overflowY:"auto",overflowX:"hidden" }}>
      ...
      {screen==="checkout"&&selectedRest&&(
        <CheckoutScreen restaurant={selectedRest} onBack={()=>setScreen("home")} onOrderPlaced={handleOrderPlaced} initialPromo={autoPromo} customerSession={customerSession} allMenuItems={menuItems}/>
      )}
      {screen==="placed"&&(
      ...
      {screen==="home"&&(
        <div style={{ height:"100%",overflowY:"auto",WebkitOverflowScrolling:"touch" }}>
          ...
          {tab==="profile"&&<ProfileScreen showToast={showToast} restaurants={restaurants} menuItems={menuItems} setRestaurants={setRestaurants} setMenuItems={setMenuItems} customerSession={customerSession} onCustomerLogout={handleCustomerLogout} onShowAuthGate={()=>setShowAuthGate(true)} onDataChange={loadAppData}/>}
        </div>
      )}
